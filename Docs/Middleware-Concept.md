...
Status: **implemented direction with historical design notes**. This document describes the
declarative middleware layer on top of the existing low-level `KompleteKontrol` driver.
The declarative `Screen` DSL is the supported client-facing API. The imperative setters
remain only as deprecated diagnostics and migration escape hatches.
...
6. **Layered, each layer usable on its own.** Diagnostics can drop down to the raw driver,
   but product integrations should stay on the declarative `Screen` DSL.
...
public final class Surface {
    ...
    // Deprecated escape hatch: imperative convenience that still goes through reconciliation
    @discardableResult
...
`setText(lcd, row, text)` was the early imperative stepping stone. It is now deprecated
for client-facing product code; prefer `Cell { Label(...) }` inside a `Screen`. Behavior:
...
- **Reactivity.** The declarative path (`Screen` / `present` / `observe`, with a SwiftUI-style
  result-builder DSL) is the supported client API. Imperative setters are deprecated escape
  hatches for diagnostics and migration.
...
- Deprecated imperative setters: `setText` with `TextOverflow` (`clip` / `ellipsis` / `fit` /
  `marquee` in `wrap` and `pingPong` styles), plus `setGlyphs`, `setBar`, and coalesced
  `setKey` / `setButtonLED`.
- Display 0 status API (`setStatus`, `setPage`).
- Declarative `Screen` / `present` / `observe` path. `show` and direct setters remain for
  legacy/diagnostic use only.
- `SurfaceDemo` executable.
- `ccd` daemon executable, separate from the baseline `KontrolProbe` diagnostic client.
...
- LED reconciler with `LampState` (`off`/`on`/`blink`/`pulse`) animated on the clock. Client
  screens should declare LEDs with `Lamp`; deprecated `setLamp` remains as a low-level escape hatch.
- `TransportState` + `updateTransport`: legacy imperative helpers for mirroring transport state
  on the hardware LEDs. Product code should model transport in a declarative `Screen`.
...
- Screens are pure values that lower to a `SurfaceModel`; `present(screen)` reconciles to it.
  The deprecated imperative setters also lower to that model, but the DSL must not depend on
  them and new app integrations should not use them.
- `SurfaceDemo` includes declarative pages (`ActivityScreen`, transport, keybed) that should be
  treated as the examples to follow. Older imperative pages are legacy demonstrations.
